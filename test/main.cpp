...
void loop_drawSomeText()
{
    ...
    FC_Font* font_texture = FC_CreateFont();
    FC_Font* font_fallback = FC_CreateFont();
    ...
    style.fontSize = 22;    style.color = FC_MakeColor(255, 255, 255, 255);         style.style = TTF_STYLE_NORMAL;
    style.outline = 1;      style.outline_color = FC_MakeColor(120, 120, 120, 255); style.thickness = 1;
    style.texture = t;      style.textureWidth = 1;                                 style.textureHeight = 16;
    FC_LoadFontFromStyle(font_texture, renderer, "./fonts/NanumGothic.ttf", &style);
    FC_SetSpacing(font_texture, -2);

    FC_LoadFont(font_fallback, renderer, "fonts/NanumGothic.ttf", 18, FC_MakeColor(0, 200, 0, 255), TTF_STYLE_NORMAL);
#endif
    ...
    while (!done)
    {
        ...
        if (mode == 0 || mode == 1)
        {
            ...
            FC_SetFallback(font_fallback, font2);
            FC_Draw(font_fallback, target, 0, 20, "UTF-8 text(fallback): %s", utf8_string);
            FC_SetFallback(font_fallback, 0);
            ...
            FC_Draw(font_texture, target, 400, 450, "Outline & Textured Text");
        }
    ...
    FC_FreeFont(font2);
    FC_FreeFont(font3);
    FC_FreeFont(font_texture);
    FC_FreeFont(font_fallback);
}
